...
export interface NodePathTestSetup { //>
...
/**
 * Sets up the common environment for NodePathService tests.
 * Must be called *within* a `describe` block.
 * @returns An object containing resolved services.
 */
export function setupNodePathTests(): NodePathTestSetup { //>
	const testContext: Partial<NodePathTestSetup> = {}

	beforeEach(async () => {
		// Clear instances first to ensure fresh resolution
		if (testContext.simulator) { // If simulator exists from a previous test run
		...
		// Re-resolve services after clearing instances
		testContext.simulator = container.resolve<IVSCodeAPISimulatorService>('IVSCodeAPISimulatorService')
		// testContext.utilsService is assigned below using the 'utils' instance
	...
	return testContext as NodePathTestSetup
